...
export default function InventoryPage() {
  ...
  const openAdd  = () => { setEditingProduct(null); setSheetOpen(true) }
  const openEdit = (p: ProductInventoryDto) => { setEditingProduct(p); setSheetOpen(true) }
  const closeSheet = () => { setSheetOpen(false); setEditingProduct(null) }
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-3 flex items-center gap-2 shrink-0">
        ...
        {tab === 'slabs' ? (
          <>
            ...
            <Button size="sm"><Plus className="w-4 h-4" />Add Slab</Button>
          </>
        ) : (
          <Button size="sm" onClick={openAdd}><Plus className="w-4 h-4" />Add Product</Button>
        )}
      </div>
      ...
      {tab === 'products' && (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {!isPending && !isError && (
              <ProductInventoryTable products={products} onEdit={openEdit} />
            )}
          </div>

          <AddProductSheet open={sheetOpen} onClose={closeSheet} editing={editingProduct} />
        </>
      )}
    </div>
  )
